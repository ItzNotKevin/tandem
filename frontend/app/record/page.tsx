...
export default function RecordPage() {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/session/upload`, {
        method: "POST",
        body: formData,
      });

      if (response.ok) {
        setUploadStatus("Successfully added to session!");
              ...
              <div className="flex flex-col items-center justify-center pt-5 pb-6">
                {isUploading ? (
                  <div className="flex flex-col items-center">
                    <div className="w-8 h-8 border-2 border-[#A6977F] border-t-transparent rounded-full animate-spin mb-4" />
                    <p className="text-sm text-[#736859]">{uploadStatus}</p>
                  </div>
                ) : (
                  <>
                    <svg
                    ...
                    <p className="mb-2 text-sm text-[#736859]">
                      {uploadStatus || (<><span className="font-semibold">Click to upload</span> or drag and drop</>)}
                    </p>
