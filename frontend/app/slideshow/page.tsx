...
export default function SlideshowPage() {
  ...
  useEffect(() => {
    ...
    fetch(`${backendUrl}/lesson`)
      .then((r) => r.ok ? r.json() : null)
      .then((data) => {
        if (data?.slides?.length) setDynamicSlides(data.slides)
      })
      .catch(() => {})
  }, [])
    ...
    <div className="min-h-screen flex flex-col" style={{ backgroundColor: '#faf6ef', fontFamily: 'Georgia, serif' }}>
      ...
      <main className="flex-1 flex flex-col px-12 pt-10 pb-6 gap-6 max-w-5xl mx-auto w-full">
        ...
        <div className="flex flex-col lg:flex-row items-stretch gap-12 w-full flex-1">
          ...
          {/* Diagram */}
          <div
            className="w-full lg:w-[26rem] rounded-2xl flex items-center justify-center shrink-0 p-4 overflow-hidden"
            style={{ backgroundColor: '#f5ede0', border: '1px solid #e0d5c5' }}
          >
            {slide.imageUrl ? (
              <img src={slide.imageUrl} alt={slide.title} className="max-w-full max-h-full object-contain" />
            ) : slide.diagram ? (
              slide.diagram
            ) : (
              <div className="w-full h-40 flex items-center justify-center">
                <span className="text-4xl font-serif italic" style={{ color: '#c9b99a' }}>{slide.title[0]}</span>
              </div>
            )}
          </div>
      ...
      <footer className="flex items-center justify-between px-12 py-7 border-t" style={{ borderColor: '#e0d5c5' }}>
        <button
          ...
          disabled={current === 0 || activeSlides.length === 0}
        ...
        <button
          ...
          style={{
            ...
            fontFamily: 'sans-serif',
            cursor: current === activeSlides.length - 1 ? 'not-allowed' : 'pointer',
          }}
        >
          Next →
        </button>
